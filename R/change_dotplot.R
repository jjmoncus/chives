...
#' @param sort_by The name of the variable to sort y-axis labels by. Can be any
...
change_dotplot <- function(
  ...
  p <- data |>
    ...
    geom_segment(
      ...
      linewidth = 1,
      color = "#6F7878",
      arrow = arrow(length = unit(.075, "in"), ends = "last", type = "closed")
    ) +
    # current value dots
    geom_point(
      aes(x = !!quo_from, y = eval(y_expr)),
      fill = "#002856",
      color = "#002856",
      size = dot_size,
      shape = 21,
  ...
  if (change_col) {
    ...
    col <- geom_text(
      mapping = aes(
        x = x_breaks[length(x_breaks)] + change_col_x,
        # if change_col = TRUE,
