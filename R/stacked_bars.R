...
stacked_bars <- function(
  ...
  quo_y <- enquo(y)
  sort_by <- arg_match(sort_by, values = c("first", "total"), multiple = FALSE)
  ...
  data %>%
    ...
    ggplot() +
    geom_col(
      aes(x = total, y = reorder(!!quo_y, desc(ordering))),
      fill = second_fill,
      width = bar_width
    ) +
    ...
    geom_col(
      aes(x = !!quo_first, y = reorder(!!quo_y, !!quo_first)),
      fill = first_fill,
      width = bar_width
    ) +
    geom_text(
